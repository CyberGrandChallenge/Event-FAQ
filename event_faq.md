...
We think we can recompile Replacement Binaries with a new compiler and we're unsure our competition will be able to reconstruct logic generated by our compiler.
*No access to federal information system- DECREE system calls*

We use exotic instructions in our RB as part of our protection mechanism; we're concerned our opponents may not be able to emulate these instructions. Are we in violation of A135 or the Conduct section of the Event Participation Agreement?
*No access to federal information system- DECREE system calls*

Some of our code paths intentionally consume a lot of resources within the DECREE jail
*No access to federal information system- DECREE system calls*

We modeled a program analysis environment named PROGA and attempted to disrupt it by packing serialized code that would escape PROGA via a software flaw in PROGA and then run code outside the DECREE jail in a Linux environment.
*Access to federal information system- non-DECREE system calls*

We modeled a program analysis environment named PROGA and attempted to disrupt it by launching a chain of gadgets that would initiate opponent CRS shutdown outside of PROGA via a software flaw in PROGA.
*Access to federal information system- non-DECREE system calls*
